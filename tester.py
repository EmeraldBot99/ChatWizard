...
class RegisterScreen(Screen):
    ...
    def go_to_login(self, instance):
        ...
        thread = threading.Thread(target=message_listener, args=(USERNAME, user_data), daemon=True)
        thread.start()
    ...
    def login_user(self, **kwargs):
        ...
        # print(f"set user_store to {USER_STORE}")
        return(user_store)

        

def message_listener(username, user_data):
    while True:
        messages = check_mailbox(user_data,username)
        print("checking mailbox...")
        ...
        if (len(messages) > 0):
            for message in messages:
                print(f"{message[1]} says: {message[0]}")
                store_message_history(message[1],username, message[0], False)
...
class MessageScreen(Screen):
    def __init__(self, **kwargs):
        ...
        conv_scroll = ScrollView(size_hint=(0.3, 1)) 
        conv_list = BoxLayout(orientation='vertical', size_hint_y=None, spacing=10, padding=10)
        conv_list.bind(minimum_height=conv_list.setter('height'))
        conv_scroll.add_widget(conv_list)
        root.add_widget(conv_scroll)

        chat_pane = BoxLayout(size_hint=(0.7, 1))
        ...
        for name in conversations:
            ...
            if message_history: 
                ...
                tile = ConversationTile(name, last_message)
                tile.bind(on_touch_down=lambda inst, touch, tile=tile: self.on_tile_click(tile, touch))
    ...
    def on_tile_click(self, tile, touch):
        if tile.collide_point(*touch.pos):
            print(f"Clicked {tile.children[1].text}") 
...
#         print("Error: USERNAME or USER_STORE not initialized")
#         return
        
#     print(f"Updating messages for user: {USERNAME}")
...
#             loaded = load_session(user_store, contact_id, f"{USERNAME}.json")
#             if not loaded:
#                 print(f"No existing session with {contact_id}, trying to establish one...")
#                 try:
#                     establish_session(user_store, contact_id)
#                     store_session(user_store, contact_id, f"{USERNAME}.json")
#                     print(f"Session established with {contact_id}")
#                 except Exception as e:
#                     print(f"Failed to establish session with {contact_id}: {e}")
        
#         new_messages = check_mailbox(user_store, USERNAME)
#         print(f"Found {len(new_messages)} new messages")
        
#         for message in new_messages:
#             print(f"Processing message from {message[1]}")
#             store_message_history(USERNAME, message[1], message[0], False)
